...
public class Reader {
        ...
        try {
            if (line[0].equals("")) {
                ...
                if (line[0].equals("update") && line[1].matches("\\d+") && line[2].equals("{element}")) {
                    ...
                    throw new InputException("Команда update должна быть вида update id {element}, где id - число");
                } else {
                    throw new InputException("Такой комбинации команды нет, используйте help для вывода списка команд");
                }
            }
    ...
    private static void toCommand(String command, Object parameter) {
        ...
        executableCommand.startCommand(command);
    }
}